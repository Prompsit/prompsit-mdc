#!/usr/bin/env python3
"""Build + validate Croissant 1.0 metadata for the D3 open layer.

Open layer = dev.jsonl (full references) + test.input.jsonl + contrastive.dev.jsonl.
sha256 inline, mlcroissant-validated, dev record-load smoke test. ASCII-only.
"""
...
FIELDS = [
    ...
    ("format", "sc:Text", "format", "Resource format profile (xml/json/properties/arb)."),
    ("source", "sc:Text", "source", "EN resource fragment."),
    ("reference", "sc:Text", "reference", "Localized resource fragment (dev only)."),
...
def build():
    ...
    meta = {
        ...
        "description": (
            "D3 - Structured-resource integrity. Real i18n resource fragments "
            "(Android string resources, Apache-2.0) in four format profiles "
            "(xml/json/properties/arb), EN -> 9 languages, where the resource "
            "skeleton (keys, schema) must be preserved and only values translated. "
            "Open layer = dev (full references) + test inputs + contrastive pack; "
            "test/hidden references withheld. Version 1.0 uses oracle-validated machine labels "
        ...
        "citeAs": f"Prompsit D3 - Structured-resource Integrity (v{manifest['version']}, 2026).",
        "distribution": [
            ...
            {"@type": "cr:FileObject", "@id": "test.input.jsonl", "name": "test.input.jsonl",
             "description": "Test inputs (references withheld).", "contentUrl": "data/test.input.jsonl",
        ...
        "recordSet": [
            {"@type": "cr:RecordSet", "@id": "dev", "name": "dev",
             "description": "Dev records.", "field": [field("dev", *f, "dev.jsonl") for f in FIELDS]},
