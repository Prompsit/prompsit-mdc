#!/usr/bin/env python3
"""D4 integrity check + manifest finalize. ASCII-only."""
...
REQUIRED = ["item_id", "source_lang", "target_lang", "profile", "source",
            "source_signature", "failure_opportunity_tags", "split", "provenance"]
...
def main():
    ...
    if [i for i, ls in langs_by.items() if ls != set(D.LANGS)]:
    ...
    # cross-split leakage + source uniqueness: a source document must not appear
    # in more than one split, and every item must be a distinct document.
    src_splits = defaultdict(set)
    for r in refs:
        src_splits[r["source"]].add(r["split"])
    leaked = [s for s, ss in src_splits.items() if len(ss) > 1]
    if leaked:
        errs.append(f"cross-split source leakage: {len(leaked)} source(s) in >1 split")
    distinct_src = len({r["source"] for r in refs})
    if distinct_src != len(langs_by):
        errs.append(f"duplicate source documents: {distinct_src} distinct vs {len(langs_by)} items")

    man = json.loads((PKG / "manifest.json").read_text(encoding="utf-8"))
    floor = man["per_class_floor"]
    # per-class floor on RECORDS in the K1 scored set (dev+test)
    k1_split = {"dev", "test"}
    per_class, seen = Counter(), set()
    for r in refs:
        seen.add(r["item_id"])
        if r["split"] in k1_split:
            for c in r["failure_opportunity_tags"]:
                per_class[c] += 1
    below = {c: per_class.get(c, 0) for c in D.ALL_CLASSES if per_class.get(c, 0) < floor}
    if below:
        errs.append(f"classes below K1 record floor {floor}: {below}")
    fail = sum(1 for r in refs if not D.score_item(r, r["reference"])["pass"])
    if fail:
