#!/usr/bin/env python3
"""D5 integrity check + manifest finalize. ASCII-only."""
...
REQUIRED = ["item_id", "source_lang", "target_lang", "kind", "source", "ref_term",
            "failure_opportunity_tags", "split", "provenance", "resources"]
    ...
    for r in refs:
        ...
        if r["split"] in k1_split:
            for c in r["failure_opportunity_tags"]:
                per_class[c] += 1
    below = {c: per_class.get(c, 0) for c in L.ALL_CLASSES if per_class.get(c, 0) < floor}
    if below:
        errs.append(f"classes below K1 record floor {floor}: {below}")
    fail = sum(1 for r in refs if not L.score_item(r, r["reference"])["pass"])
    if fail:
