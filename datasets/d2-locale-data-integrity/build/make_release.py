...
def main():
    ...
    # checksums.sha256 over the published files (sorted, deterministic)
    lines = []
    ...
    # open-layer archive (mtime fixed for reproducibility)
    ...
    with tarfile.open(archive, "w:gz", compresslevel=9) as tar:
        for rel in sorted(present):
            ...
            info.mtime = 1750000000  # fixed epoch -> reproducible bytes
            info.uid = info.gid = 0
